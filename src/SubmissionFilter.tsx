...
import { type MonthDayYear } from "./DateRange";
...
type SubmissionFilterProps = {
  filter?: SubmissionFilterValue;
  onFilterPressed?: (query: SubmissionFilterValue) => void;
  onCloseClicked?: () => void;
};

type SubmissionFilterState = SubmissionFilterValue & {
  showWhen: boolean;
...
const TypedAddressView = AddressView as React.ComponentType<any>;

export default class SubmissionFilter extends React.Component<SubmissionFilterProps, SubmissionFilterState> {
  private _address?: { blur: () => void } | null;
  private _complaint?: unknown;

  constructor(props: SubmissionFilterProps) {
    super(props);
  ...
  filterPressed() {
    ...
    const query: SubmissionFilterValue = {
      keywords,
      location,
      complaints,
      srid,
    ...
    if (this.state.showComplaintModal) {
      ...
      return (
        ...
        >
          <ComplaintView
            complaints={[]}
            onComplaintsChanged={(complaints: Category[]) => {
  ...
  finds(address: AddressComponent[], key: string) {
  ...
  get addressModal() {
    if (this.state.showAddressModal) {
      return (
        ...
        >
          <TypedAddressView
            onPress={({ place: location }: { data: unknown; place: FilterLocation }) => {
              this._address?.blur();
              this.setState({
                location: location,
  ...
  render() {
    return (
      <>
        ...
        <TypedCard title={"Search Reports"} style={{ backgroundColor: undefined }}>
          <View style={styles.container}>
            <View style={styles.horiz}></View>
            <Input
              placeholder={""}
              label="Keywords"
            ...
            <View>
              ...
              >
                <Input
                  ...
                  caretHidden={true}
                  autoFocus={false}
                  onFocus={() => this.setState({ showComplaintModal: true })}
                  label={"Complaint"}
            ...
            <View>
              ...
              >
                <Input
                  ref={(r: any) => (this._address = r)}
                  caretHidden={true}
                  autoFocus={false}
                  onFocus={() => this.setState({ showAddressModal: true })}
                  label={"Address"}
