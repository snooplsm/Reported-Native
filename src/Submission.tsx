...
export default function Submission({ navigation }: SubmissionProps) {
  const [stateMedia, setMedia] = React.useState<any[]>([]);
  const [stateDatePickerVisible, setDatePickerVisible] = React.useState<any>(undefined);
  const [stateTimeofreport, setTimeofreport] = React.useState<any>(undefined);
  const [stateTimeofreportstr, setTimeofreportstr] = React.useState<string | undefined>(undefined);
  const [stateTimeofreportExif, setTimeofreportExif] = React.useState<any>(undefined);
  const [stateComplaints, setComplaints] = React.useState<any[]>([]);
  const [stateImageModal, setImageModal] = React.useState<number | undefined>(undefined);
  const [stateUploadedMedia, setUploadedMedia] = React.useState<Record<string, any>>({});
  ...
  const [stateLicense, setLicense] = React.useState<any>(undefined);
  const [stateAlpr, setAlpr] = React.useState<any>(undefined);
  const [stateAlprImage, setAlprImage] = React.useState<any>(undefined);
  const [stateLocation, setLocation] = React.useState<any>(undefined);
  const [stateKeyboard, setKeyboard] = React.useState<boolean | undefined>(undefined);
  ...
  const _complaint = React.useRef<any>(null);

  // const navigation = useNavigation();
  // console.log('subm navi', navigation);
  const auth = useAuth();

  const draftKey = `report.draft.${(Constants as any).nativeAppVersion}`;
  ...
  const setState = async (state: any, lambda?: () => void) => {
    if (modalsShowing()) {
  ...
  const alrt = (title: string, message: string) => {
    setSubmitting(false);
  ...
  const progressListener = (progress: any) => {
    const promise = new Promise(function (resolve) {
      const reducer = (sum: number, num: any) => {
        return sum + num.size;
      };
      const plate =
        stateLicense &&
      ...
      const loaded =
        Object.values(stateUploadedMedia).reduce(reducer, 0) + progress.loaded;
      resolve({ total, loaded });
    });
    promise
      .then(({ total, loaded }: any) => {
  ...
  const getLocationData = async (imageLocation: any) => {
  ...
  const setTimeOfReportField = (timeofreport: any) => {
    ...
    if (!exif) {
      ...
      if (!stateLicense) {
        ...
        if (image.width > image.height) {
          resize = { width: Math.min(1200, image.width) };
        } else {
          resize = { height: Math.min(1200, image.height) };
        }
        const data: any = {
          original: image
        };
