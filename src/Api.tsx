...
function getApiUrl() {
    ...
    const channel = Updates.channel ?? (Constants.manifest as any)?.releaseChannel;
...
const uploadImageOnS3 = (key: string, contentType: string, file: Blob) => {
...
export const uploadFile = async (user: any, file: any, extra?: any): Promise<any> => {
...
class UserPromise extends Promise<any> {
    constructor(res: any) {
        super(resolve => {
            AsyncStorage.setItem(USER_KEY, JSON.stringify(res.data)).then(() => {
                resolve(res);
            });
        });
    }
...
export const geocode = address => {
    ...
    return AsyncStorage.getItem(key).then(item => {
        ...
        const url = `https://maps.googleapis.com/maps/api/geocode/json?key=${encodeURIComponent(env.googleMapsApiKey)}&address=${encodeURIComponent(
            address
        )}`;
        return fetch(url)
            .then(res => res.json())
...
export const reverseGeocode = location => {
    const url = `https://maps.googleapis.com/maps/api/geocode/json?key=${encodeURIComponent(env.googleMapsApiKey)}&latlng=${location.lat},${location.lng}&result_type=street_address&rankBy=distance`;
    return fetch(url)
        .then(res => res.json())
        .then(data => {
            return data;
...
export async function pushTokenNeedsRegisteringAsync() {
    ...
    let token2 = await getExpoPushTokenAsync();
    return token2 !== token;
}
